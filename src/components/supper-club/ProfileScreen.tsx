...
const AVATAR_COLORS = [
  "#d4cdc4", "#7a9e7e", "#9b7ec8", "#c45c5c", "#4a8bc2", "#c4a35c",
...
export default function ProfileScreen({ user, userReviews, userBadges, onClose, showToast, signOut }: ProfileScreenProps) {
  ...
  const [avatarColor, setAvatarColor] = useState(user.user_metadata?.avatar_color || "#d4cdc4");
  ...
  useEffect(() => {
    supabase.from("profiles").select("bio, city, avatar_url, avatar_color, display_name").eq("id", user.id).maybeSingle().then(({ data }) => {
      if (data) { setBio(data.bio || ""); setCity(data.city || ""); setAvatarUrl(data.avatar_url || null); if (data.avatar_color) setAvatarColor(data.avatar_color); if (data.display_name) setDisplayName(data.display_name); }
    });
  }, [user.id]);
  ...
  const handleSave = async () => {
    ...
    setSaving(false); showToast("Profile updated!"); setEditing(false);
  };
        ...
        <div style={{ padding:"24px 16px 0" }}>
          <div style={{ display:"flex", justifyContent:"center", marginBottom:"20px" }}>
            <div style={{ position:"relative" }}>
              <div onClick={() => editing && fileInputRef.current?.click()} style={{ width:"96px", height:"96px", borderRadius:"50%", background:avatarUrl?`url(${avatarUrl}) center/cover no-repeat`:avatarColor, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"32px", color:"#fff", fontWeight:"700", border:"3px solid rgba(212,205,196,0.25)", boxShadow:"0 4px 24px rgba(0,0,0,0.4)", cursor:editing?"pointer":"default", overflow:"hidden" }}>
                {!avatarUrl && displayName.charAt(0).toUpperCase()}
                {uploading && <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.6)", display:"flex", alignItems:"center", justifyContent:"center", borderRadius:"50%" }}><div style={{ width:"20px", height:"20px", border:"2px solid rgba(212,205,196,0.3)", borderTop:"2px solid #d4cdc4", borderRadius:"50%", animation:"spin 1s linear infinite" }} /></div>}
              </div>
              {editing && <div onClick={() => fileInputRef.current?.click()} style={{ position:"absolute", bottom:0, right:0, width:"28px", height:"28px", borderRadius:"50%", background:"#d4cdc4", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", border:"2px solid #2a2a2a", fontSize:"14px", color:"#2a2a2a", fontWeight:"700" }}>+</div>}
          ...
          {!editing ? (
            <>
              ...
              <div style={{ display:"flex", justifyContent:"center", gap:"32px", marginBottom:"24px", padding:"16px 0", borderTop:"1px solid rgba(212,205,196,0.08)", borderBottom:"1px solid rgba(212,205,196,0.08)" }}>
                ...
                <div style={{ textAlign:"center" }}><div style={{ fontSize:"20px", color:"#e5ded5", fontWeight:"600" }}>{[...new Set(userReviews.map(r => r.restaurant_name))].length}</div><div style={{ fontSize:"10px", color:"#8c8278", letterSpacing:"1px", textTransform:"uppercase" }}>Restaurants</div></div>
                <div style={{ textAlign:"center" }}><div style={{ fontSize:"20px", color:"#e5ded5", fontWeight:"600" }}>{userReviews.length > 0 ? (userReviews.reduce((s, r) => s + r.rating, 0) / userReviews.length).toFixed(1) : "—"}</div><div style={{ fontSize:"10px", color:"#8c8278", letterSpacing:"1px", textTransform:"uppercase" }}>Avg Rating</div></div>
              </div>
              ...
              <div style={{ marginTop:"24px" }}>
                <div style={{ fontSize:"11px", color:"#d4cdc4", letterSpacing:"2px", textTransform:"uppercase", marginBottom:"12px" }}>Your Reviews</div>
                {reviewCities.length > 1 && (
                  <div style={{ display:"flex", gap:"6px", marginBottom:"14px", flexWrap:"wrap" }}>
                    <div style={chip(reviewFilter === "all")} onClick={() => setReviewFilter("all")}>All</div>
                    {reviewCities.map(c => <div key={c} style={chip(reviewFilter === c)} onClick={() => setReviewFilter(c)}>{c}</div>)}
                  </div>
                )}
                {filteredReviews.length === 0 ? (
                  <div style={{ textAlign:"center", padding:"32px 16px" }}>
                    <div style={{ fontSize:"20px", color:"#3d3d3d", marginBottom:"8px" }}>◈</div>
                    <div style={{ fontSize:"13px", color:"#8c8278", fontStyle:"italic" }}>No reviews yet. Your culinary critiques will appear here.</div>
                  </div>
              ...
              {!avatarUrl && (
                <div style={{ marginBottom:"20px" }}>
                  ...
                  <div style={{ display:"flex", gap:"10px", flexWrap:"wrap" }}>
                    {AVATAR_COLORS.map(c => <div key={c} onClick={() => setAvatarColor(c)} style={{ width:"32px", height:"32px", borderRadius:"50%", background:c, cursor:"pointer", border:avatarColor===c?"3px solid #e5ded5":"2px solid rgba(212,205,196,0.15)" }} />)}
                  </div>
                </div>
              )}
              <div style={{ marginBottom:"16px" }}><label style={S.label}>First Name</label><input style={S.input} value={displayName} onChange={e => setDisplayName(e.target.value)} placeholder="Your name" /></div>
